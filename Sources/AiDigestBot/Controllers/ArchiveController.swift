...
//        }.joined(separator: "\n")
        ...
        if records.isEmpty {
            entriesHTML = "<p>No digests yet — check back after the first run.</p>"
        } else {
            entriesHTML = records.map { record in
                """
                <article class="digest-entry">
                    <h2>\(record.date)</h2>
                    <p class="paper-count">\(record.paperTitles.count) papers covered</p>
        ...
        let templateURL = URL(fileURLWithPath: req.application.directory.resourcesDirectory)
            .appendingPathComponent("digest-archive.html")
        let template = try String(contentsOf: templateURL, encoding: .utf8)
        let html = template.replacingOccurrences(of: "{{ENTRIES}}", with: entriesHTML)
